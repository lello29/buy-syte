  ...
  async insert(table: string, data: any) {
    try {
      ...
      if (error) {
        ...
        toast.error(`Errore nell'inserimento dei dati: ${(error as PostgrestError).message || 'Errore sconosciuto'}`);
        return null;
      }
  ...
  async delete(table: string, column?: string, value?: any) {
    try {
      ...
      if (error) {
        ...
        toast.error(`Errore nell'eliminazione dei dati: ${(error as PostgrestError).message || 'Errore sconosciuto'}`);
        return false;
      }
